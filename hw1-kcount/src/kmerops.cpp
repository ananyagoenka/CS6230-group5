...
std::unique_ptr<KmerList> count_kmer(const DnaBuffer &myreads)
{
    // Check if environment variable is set to determine which implementation to use
    ...
    if (impl == "omp") {
        ...
        // This is the original count_kmer implementation
        Timer timer;
...
//     // Skip execution if only one process
//     if (size == 1) {
//         return count_kmer(myreads);
//     }

//     // OPTIMIZATION 1: Faster local hash map construction
//     // Direct hash map without indirection or template overhead
//     std::unordered_map<KmerSeedStruct, int, KmerSeedHash> local_kmermap;
...
//         size_t estimated_kmers = (total_bases - KMER_SIZE + 1) / 4;
//         local_kmermap.reserve(estimated_kmers);
//     }
    
//     // Direct counting without handler indirection
//     for (size_t i = 0; i < myreads.size(); ++i) {
//         if (myreads[i].size() < KMER_SIZE)
//             continue;
...
//     // OPTIMIZATION 2: Use simplified owner assignment with modulo hash
//     // Create direct arrays for each process
//     std::vector<int> send_counts(size, 0);
//     std::vector<std::vector<KmerSeedStruct>> kmer_by_rank(size);
//     std::vector<std::vector<int>> count_by_rank(size);
    
//     // Count elements going to each rank first
//     for (const auto& entry : local_kmermap) {
//         int target_rank = GetKmerOwner(entry.first.kmer, size);
//         send_counts[target_rank]++;
//     }
    
//     // Pre-allocate with exact sizes
//     for (int i = 0; i < size; i++) {
//         kmer_by_rank[i].reserve(send_counts[i]);
//         count_by_rank[i].reserve(send_counts[i]);
//     }
    
//     // Distribute kmers to target ranks
//     for (const auto& entry : local_kmermap) {
//         int target_rank = GetKmerOwner(entry.first.kmer, size);
//         kmer_by_rank[target_rank].push_back(entry.first);
//         count_by_rank[target_rank].push_back(entry.second);
//     }
    
//     // Free local map immediately
//     local_kmermap.clear();
//     std::unordered_map<KmerSeedStruct, int, KmerSeedHash>().swap(local_kmermap);
    
//     // OPTIMIZATION 3: More efficient communication pattern
...
//     // Calculate total receive counts
//     int total_recv_kmers = 0;
//     for (int i = 0; i < size; i++) {
//         total_recv_kmers += recv_counts[i];
//     }
...
//     // For KmerSeedStruct
//     MPI_Datatype kmer_seed_type;
//     MPI_Type_contiguous(sizeof(KmerSeedStruct), MPI_BYTE, &kmer_seed_type);
//     MPI_Type_commit(&kmer_seed_type);
    
//     // OPTIMIZATION 4: Single buffer transfers
//     // Create flattened send arrays
//     std::vector<KmerSeedStruct> send_kmers;
//     std::vector<int> send_counts_flat;
    
//     for (int i = 0; i < size; i++) {
//         send_kmers.insert(send_kmers.end(), kmer_by_rank[i].begin(), kmer_by_rank[i].end());
//         send_counts_flat.insert(send_counts_flat.end(), count_by_rank[i].begin(), count_by_rank[i].end());
        
//         // Free memory immediately
//         std::vector<KmerSeedStruct>().swap(kmer_by_rank[i]);
//         std::vector<int>().swap(count_by_rank[i]);
//     }
    
//     // Clear parent vectors
//     std::vector<std::vector<KmerSeedStruct>>().swap(kmer_by_rank);
//     std::vector<std::vector<int>>().swap(count_by_rank);
...
//     // Transfer data
//     MPI_Alltoallv(
//         send_kmers.data(), send_counts.data(), send_displs.data(), kmer_seed_type,
//         recv_kmers.data(), recv_counts.data(), recv_displs.data(), kmer_seed_type,
//         MPI_COMM_WORLD
//     );
...
//     // Free datatype and send buffers
//     MPI_Type_free(&kmer_seed_type);
//     send_kmers.clear();
//     send_counts_flat.clear();
//     std::vector<KmerSeedStruct>().swap(send_kmers);
//     std::vector<int>().swap(send_counts_flat);
    
//     // OPTIMIZATION 5: Fast final merging
//     // Final map with perfect sizing
//     std::unordered_map<KmerSeedStruct, int, KmerSeedHash> final_kmermap;
//     final_kmermap.reserve(total_recv_kmers);
    
//     // Merge with direct insertion
//     for (int i = 0; i < total_recv_kmers; i++) {
//         final_kmermap[recv_kmers[i]] += recv_counts_flat[i];
//     }
