#!/usr/bin/env python3
"""
Script to run PageRank benchmarks comparing traditional CPU, single-GPU sparse matrix,
...
from src.algorithms.multi_gpu_pagerank import MultiGPUPageRank
...
def run_strong_scaling_test(base_size, num_gpus_list, graph_type='scale-free', seed=42, runs=3):
    ...
    # Convert graph to adjacency list for traditional algorithms
    ...
    # Run traditional CPU PageRank
    print("\nRunning traditional PageRank on CPU...")
    try:
    ...
    # Run single-GPU sparse PageRank
    print("\nRunning optimized sparse PageRank on single GPU...")
    try:
        ...
        print(f"Error in single-GPU optimized PageRank: {e}")
    
    # Run multi-GPU PageRank for each GPU count
    for num_gpus in num_gpus_list:
        if num_gpus > torch.cuda.device_count():
            print(f"\nSkipping {num_gpus} GPUs test (only {torch.cuda.device_count()} available)")
            continue
            
        print(f"\nRunning multi-GPU PageRank with {num_gpus} GPUs...")
        try:
            ...
            multi_gpu_result, _ = run_test(
                MultiGPUPageRank.la_pagerank_multi_gpu_v2,
                adj_matrix_sparse,
                damping=damping,
                max_iterations=max_iterations,
                tol=tol,
            ...
            print(f"Error in multi-GPU PageRank with {num_gpus} GPUs: {e}")
...
def run_weak_scaling_test(base_size, num_gpus_list, graph_type='scale-free', seed=42, runs=3):
    ...
    for num_gpus in num_gpus_list:
        ...
        # Run traditional CPU PageRank if graph is not too large
        if graph_size <= 100000:  # Limit for CPU tests
            ...
            print("\nSkipping traditional PageRank on CPU (graph too large)")
        ...
        try:
            ...
            print(f"Error in multi-GPU PageRank: {e}")
...
def plot_strong_scaling_results(results, output_dir=None):
    ...
    plt.title('PageRank Execution Time vs. Number of GPUs')
    plt.grid(True)
    plt.legend()
    ...
    if output_dir:
        ...
        plt.savefig(os.path.join(output_dir, 'pagerank_strong_scaling.png'), dpi=300)
        plt.close()
    else:
        plt.show()
...
def plot_weak_scaling_results(results, output_dir=None):
    ...
    plt.ylabel('Execution Time (s)')
    plt.title('PageRank Weak Scaling: Execution Time vs. Number of GPUs')
    plt.grid(True)
    plt.legend()
    ...
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)
        plt.savefig(os.path.join(output_dir, 'pagerank_weak_scaling.png'), dpi=300)
        plt.close()
    else:
        plt.show()
...
def main():
    ...
    parser.add_argument('--max-iterations', type=int, default=100,
                        help='Maximum number of iterations for PageRank (default: 100)')
    parser.add_argument('--tolerance', type=float, default=1e-6,
                        help='Convergence tolerance for PageRank (default: 1e-6)')
    ...
    output_dir = os.path.join(args.output_dir, f'pagerank_{args.graph_type}_{timestamp}')
    os.makedirs(output_dir, exist_ok=True)
