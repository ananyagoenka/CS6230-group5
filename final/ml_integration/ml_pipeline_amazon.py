#!/usr/bin/env python
"""
ML Pipeline with BFS and PageRank for graph datasets including Amazon and Reddit
Modified to support all datasets with proper train/val/test masks
"""
...
# Helper function to preprocess datasets that don't have masks
def preprocess_dataset(data, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2):
    """Add train/val/test masks to datasets that don't have them"""
...
class BFSPipelineCPU(BFSPipeline):
    def process_graph(self, data):
        ...
        # Select a central node for BFS (or sample multiple nodes for large graphs)
        start_node = 0
        if hasattr(data, 'num_nodes') and data.num_nodes > 10000:
            # For very large graphs, use a more central node or sample
            # This is especially important for Reddit
            start_node = len(adj_list) // 2  # Use middle node as a heuristic
            
        # Apply BFS from the selected node
...
class BFSPipelineGPU(BFSPipeline):
    def process_graph(self, data):
        ...
        if hasattr(data, 'num_nodes') and data.num_nodes > 10000:
            # For very large graphs, use a more central node or sample
            # This is especially important for Reddit
            start_node = data.num_nodes // 2  # Use middle node as a heuristic
...
class PageRankPipelineCPU(PageRankPipeline):
    def process_graph(self, data):
        ...
        # For large graphs, adjust PageRank parameters
        max_iterations = 100
        tolerance = 1e-6
        
        if hasattr(data, 'num_nodes') and data.num_nodes > 10000:
            # For very large graphs (like Reddit), use fewer iterations and higher tolerance
            max_iterations = 50
            tolerance = 1e-5
        
        # Apply PageRank
        pagerank_scores, _ = PageRank.traditional_pagerank_cpu(
            adj_list, 
            damping=0.85, 
...
class PageRankPipelineGPU(PageRankPipeline):
    def process_graph(self, data):
        ...
        pagerank_scores, _ = PageRank.la_pagerank_sparse_gpu_optimized_v2_turbo(
            adj_matrix, 
            damping=0.85, 
            max_iterations=max_iterations, 
            tol=tolerance
...
def run_experiment(dataset_name='Cora', gnn_type='GCN', device='cuda', epochs=100):
    ...
    if dataset_name == 'Cora':
        ...
        data = dataset[0]
        # Preprocess to add train/val/test masks
        data = preprocess_dataset(data)
    elif dataset_name == 'Reddit':
        dataset = Reddit(root='/tmp/Reddit')
        data = dataset[0]
        # Reddit should already have masks, but double check
        if not hasattr(data, 'train_mask'):
            data = preprocess_dataset(data)
    else:
    ...
    if dataset_name in ['Amazon', 'Reddit']:
        # Use larger hidden dimensions for bigger datasets
        hidden_channels = 64
    else:
        hidden_channels = 16
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='Cora', 
                      choices=['Cora', 'CiteSeer', 'PubMed', 'Reddit', 'Amazon'],
                      help='Dataset to use')
    parser.add_argument('--gnn', type=str, default='GCN', 
                      choices=['GCN', 'GAT', 'SAGE'],
                      help='GNN model to use')
    parser.add_argument('--cpu', action='store_true', 
                      help='Force using CPU only')
